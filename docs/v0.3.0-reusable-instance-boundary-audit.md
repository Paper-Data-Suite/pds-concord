...
**Issue #64 implementation note (2026-08-27):** #64 now implements that
handoff as workspace-level typed presets with immutable revisions and fresh
Activity-native materialization; the original #48 classification remains the
normative reusable-versus-instance boundary.
...
## Issue #65 implementation note

The guided **Create Classroom Activity** workflow implements this audit's reuse
boundary as orchestration only. Fresh creation, #63 copy, saved materials,
GroupPlans, presets, and assessment setup continue to materialize through their
existing authorities. **Continue setup** derives status from canonical records;
there is no persistent wizard-state authority.
