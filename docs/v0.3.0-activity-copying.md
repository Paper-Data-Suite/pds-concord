...
Issue #63 does not invent cross-Activity Criterion Set sharing. Issue #64 now
provides separate workspace-level Criterion/Scale preset authority; using it
materializes fresh target-native scoring state and does not change #63's empty
`criterion_set_ids` copy boundary.
...
**Create Classroom Activity** now offers a teacher-facing “start from another
Activity” path over `prepare_activity_copy` / `copy_activity`. The #63 positive
allowlist, zero-write preview, digest binding, fresh identities, and exclusion of
Groups, assignments, evidence, Scores, and history are unchanged.
