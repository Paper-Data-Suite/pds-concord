...
Approval remains planning-only. Issue #56 now implements the separate exact
application preview and the sole transaction that creates canonical
`Group`/`GroupMembership` state and marks an approved plan applied.
...
approved-plan application                  -> Issue #56 (implemented)
